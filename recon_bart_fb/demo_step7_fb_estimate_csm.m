% demo_step7_fb_estimate_csm.m
% Written by Nam Gyun Lee
% Email: namgyunl@usc.edu, ggang56@gmail.com (preferred)
% Started: 04/27/2024, Last modified: 09/18/2024
...
%% Estimate coil sensitivity maps per respiratory state and per echo
for b = 1%:B
    for eco = 1:nr_echoes

        %% Define filenames
        traj_unscaled_filename = sprintf('btraj%d_eco%d_unscaled', b, eco);
        traj_filename          = sprintf('btraj%d_eco%d_%dx%dx%d', b, eco, traj_scale_factor(1), traj_scale_factor(2), traj_scale_factor(3));
        cimg_filename          = sprintf('bcimg%d_eco%d_%dx%dx%d', b, eco, N1, N2, N3);
        sens_fiename           = sprintf('bsens%d_eco%d_%dx%dx%d', b, eco, N1, N2, N3);
        img_calib_filename     = sprintf('bimg%d_eco%d_calib', b, eco);
        sens_calib_filename    = sprintf('bsens%d_eco%d_calib', b, eco);

        traj_file       = strcat(bart_output_path, traj_filename);
        ksp_file        = strcat(bart_output_path, sprintf('bksp%d_eco%d', b, eco));
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d)[BART] Estimating coil sensitivities using NLINV:\n%s\n', datetime, b, B, eco, nr_echoes, command);
        ...
        % -h    help
        %------------------------------------------------------------------
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d) Transforming from image space to k-space... ', datetime, b, B, eco, nr_echoes);
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d) Zeropadding to a 2X grid in k-space... ', datetime, b, B, eco, nr_echoes);
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d) Transforming from k-space to image space... ', datetime, b, B, eco, nr_echoes);
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d) Cropping a 2X grid to a 1X grid in image space... ', datetime, b, B, eco, nr_echoes);
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d) Normalizing coil sensitivity maps estimated by NLINV... ', datetime, b, B, eco, nr_echoes);
