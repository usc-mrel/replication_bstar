...
%% Perform PICS reconstruction per respiratory state and per echo
for b = 1%:B
    for eco = 1:nr_echoes

        %% Define filenames
        traj_filename     = sprintf('btraj%d_eco%d_%dx%dx%d', b, eco, traj_scale_factor(1), traj_scale_factor(2), traj_scale_factor(3));
        weights_filename  = sprintf('bweights%d_eco%d_effMtx%d_kappa%g', b, eco, N1, kappa);
        ksp_filename      = sprintf('bksp%d_eco%d', b, eco);
        sens_fiename      = sprintf('bsens%d_eco%d_%dx%dx%d', b, eco, N1, N2, N3);
        img_pics_filename = sprintf('bimg%d_pics_eco%d_kappa%g_l%g_i%d_%dx%dx%d', b, eco, kappa, lambda, max_iter, N1, N2, N3);
        ...
        %% Read a .cfl file
        cfl_file = fullfile(output_path, sprintf('bdcf%d_eco%d_effMtx%d', b, eco, N1));
        ...
        tstart = tic; fprintf('%s:(B=%2d/%2d)(ECO=%d/%d)[BART] parallel imaging and compressed sensing reconstruction:\n%s\n', datetime, b, B, eco, nr_echoes, command);
    ...
    img_pics_combined_filename = sprintf('bimg%d_pics_combined_kappa%g_l%g_i%d_%dx%dx%d', b, kappa, lambda, max_iter, N1, N2, N3);
