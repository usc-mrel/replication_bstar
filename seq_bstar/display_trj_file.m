...
sgtitle({sprintf('Measured k-space trajectories, bandwith = %4.0f [Hz/Px], samples = %d', bandwidth1, nr_samples), ...
         sprintf('RUTime/TotalTime/ADC[0]/Resolution = %3.0f/%3.0f/%6.1f/%4.2f', ramp_time * 1e6, total_time * 1e6, adc_duration * 1e6, resolution * 1e3), ...
         sprintf('scope/bstar amplitude = %3.1f/%3.1f [mT/m], scale factor = %6.4f', amplitude_scope, amplitude_bstar, scale_factor)});

output_fullpath = fullfile(output_directory, sprintf('traj_b%4.0f_n%d_usc', bandwidth1, nr_samples));
