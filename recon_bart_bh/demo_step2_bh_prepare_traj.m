...
% hold on;
% for i = 1:nr_radial_views
%     plot3(squeeze(k_dcs(1,:,i)), squeeze(k_dcs(2,:,i)), squeeze(k_dcs(3,:,i)));
%     pause(0.01);
% end
% return
...
traj_unscaled(2,:,:) = -k_rcs(2,:,:) / (2 * krmax); % AP direction when SAGITTAL, PE along AP
...
for eco = 1:nr_echoes
    ...
    %----------------------------------------------------------------------
    % traj_unscaled (3 x adc_samples_per_echo x nr_radial_views)
    %----------------------------------------------------------------------
